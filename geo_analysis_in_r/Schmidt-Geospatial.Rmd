...
# Introduction
```{r load-hidden-packages, include=FALSE}
# initial packages
library(vembedr)
```
...
#vembedr::embed_url("url to intro video")
```
...
```{r}
# insert basic mapmaking video here
#vembedr::embed_url("url to basic mapmaking video")
```
...
Now we have a new dataset containing the top 10 most populous U.S. capitals. We can use the name variable in this dataset to create a label on our map. One of my favorite geoms to use to add labels is `geom_label_repel()`, which will draw a rectangle underneath the text and make it easier to read. The labels also get repelled from the data points and from each other!
...
```{r}
# insert OSM video here
#vembedr::embed_url("url to OSM video")
```
...
```{r}
# insert OSM challenge video here
#vembedr::embed_url("url to OSM challenge video")
```
...
```{r}
# insert online open data video here
#vembedr::embed_url("url to online open data video")
```
...
```{r}
# insert open data challenge video here
#vembedr::embed_url("url to open data challenge video")
```

Try using the avalanche coordinates to plot the exact locations for the 2022/23 hydrological year. Use the canton map we already created and `geom_point()` to plot the avalanche locations. If you want a bonus challenge, you can color the points by the number of people who died in the avalanches. 
...
```{r}
# insert elevation video here
#vembedr::embed_url("url to elevation video")
```
...
```{r}
# insert customizing video here
#vembedr::embed_url("url to customizing video")
```
...
ggsave(plot,
       filename = "./images/interlaken.png",
       width = 5,
       height = 3,
       units = "in",
       dpi = 600,
...
# save
ggsave(plot,
       filename = "./images/swiss-elevation.png",
       width = 5,
       height = 3,
       units = "in",
       dpi = 600,
...
![](./images/swiss-elevation.png)

# Next Steps

```{r}
# insert next steps video here
#vembedr::embed_url("url to next steps video")
```
